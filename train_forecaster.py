...
中文说明：
训练预测器（使用 VQVAE 代码本）。
- 统一使用 os.path.join 进行路径拼接，避免硬编码斜杠；
- 提供输出目录的合理默认值（env.get_default_output_dir）；
- 保存日志与检查点路径更稳健，提升在 Windows/Linux 的兼容性。
"""
...
from lib.utils.env import seed_all_rng, get_default_output_dir, ensure_dir
from tqdm.auto import tqdm


def create_time_series_dataloader(datapath=None, batchsize=8):
    dataloaders = {}
    if datapath in (None, ""):
        datapath = os.path.join(get_default_output_dir(), "data")
...
def train_one_epoch(
    ...
    for i, data in enumerate(tqdm(dataloader, desc=f"train epoch {epoch}", unit="batch")):
        # ----- LOAD DATA ------ #
...
def train(args):
    # 文件保存路径默认到 outputs/logs
    if args.file_save_path in (None, ""):
        args.file_save_path = os.path.join(get_default_output_dir(), "logs")
    ensure_dir(args.file_save_path)

    save_name = str(args.data_type) + '_Tin' + str(args.Tin) + '_Tout' + str(args.Tout) + '_seed' + str(args.seed) + '.txt'
    save_file = open(os.path.join(args.file_save_path, save_name), 'w+')
    ...
    # -------- PARAMS ------- #
    # 数据路径默认到 outputs/data/<data_type>
    if args.data_path in (None, ""):
        args.data_path = os.path.join(
            get_default_output_dir(),
            "data",
            args.data_type,
            f"Tin{args.Tin}_Tout{args.Tout}",
        )
    ...
    # -------- CHECKPOINT ------- #
    if args.checkpoint:
        if args.checkpoint_path in (None, ""):
            args.checkpoint_path = os.path.join(get_default_output_dir(), "checkpoints")
        ensure_dir(args.checkpoint_path)
    early_stopping = EarlyStopping(
        patience=args.patience,
        path=args.checkpoint_path if args.checkpoint else None,
    )
    ...
    for epoch in range(args.epochs):
        ...
        if val_dataloader is not None:
            ...
            with torch.no_grad():
                for i, vdata in enumerate(tqdm(val_dataloader, desc="val", unit="batch")):
                    pred_time = inference(
                        vdata,
                        model_decode,
                        model_mustd,
        ...
        if test_dataloader is not None:
            ...
            with torch.no_grad():
                ...
                for i, tdata in enumerate(tqdm(test_dataloader, desc="test", unit="batch")):
                    pred_time = inference(
                        tdata,
                        model_decode,
                        model_mustd,
...
def default_argument_parser():
    ...
    parser.add_argument(
        "--checkpoint_path", default='', type=str
    )
