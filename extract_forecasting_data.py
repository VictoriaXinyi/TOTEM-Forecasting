...
class ExtractData:
    ...
    def one_loop_forecasting(self, loader, vqvae_model, desc="loop"):
        x_original_all = []
        y_original_all = []
        x_code_ids_all = []
        y_code_ids_all = []
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(loader, desc=desc, unit="batch")):
            if i == 0:
    ...
    def extract_data(self):
        ...
        if self.args.classifiy_or_forecast == 'forecast':
            print('FORECASTING')
            # 默认保存路径：output/data/<data>/Tin<seq_len>_Tout<pred_len>/
            save_dir = self.args.save_path if isinstance(self.args.save_path, str) else ""
            if save_dir in (None, "", False):
                save_dir = os.path.join(
                    get_default_output_dir(),
                    "data",
                    str(self.args.data),
                    f"Tin{self.args.seq_len}_Tout{self.args.pred_len}",
                )
            ensure_dir(save_dir)
            self.args.save_path = save_dir

            # These have dimension [bs, ntime, nvars]
            print('-------------TRAIN-------------')
            train_data_dict = self.one_loop_forecasting(train_loader, vqvae_model, desc="train")
            save_files_forecasting(self.args.save_path, train_data_dict, 'train', save_codebook=True)

            print('-------------Val-------------')
            val_data_dict = self.one_loop_forecasting(vali_loader, vqvae_model, desc="val")
            save_files_forecasting(self.args.save_path, val_data_dict, 'val', save_codebook=False)

            print('-------------Test-------------')
            test_data_dict = self.one_loop_forecasting(test_loader, vqvae_model, desc="test")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='',
                        help='folder to save extracted forecasting data; defaults to output/data/<data>/Tin<Tin>_Tout<Tout>')
