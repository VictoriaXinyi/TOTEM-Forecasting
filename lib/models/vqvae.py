...
"""
中文说明：
VQVAE 时序编码器/解码器与向量量化模块的实现。
- Encoder/Decoder：在设定的压缩因子下对一维时间序列进行卷积式编码与解码；
- VectorQuantizer：将连续潜表示离散到代码本（codebook），并返回编码索引与嵌入；
- vqvae：继承 BaseModel，提供 shared_eval 训练/评估入口。
本文件面向时间序列场景，默认将输入按需要重排为 [batch, time] 或 [batch, channels, time]。
"""
